...
public class GenericFocus extends Focus {
	...
	/** Maps a Formula object to a FocusFormula object, which is its optimized form.
	 */
	private Map<Formula, FocusFormula> registeredFormulae = HashMapFactory.make();
	...
	public Collection<TVS> focus(TVS structure, Formula origFormula) {
		// Retrieve the optimized form of the formula, and if not found optimize it.
		...
		for (int i = 0; i < formula.focusFormulae.length; i++) {
			...
			stepIt[0] = new SingleIterator<StructureAssign>(new StructureAssign(structure, Assign.EMPTY));
			
			int currentStep = 0;
			while (currentStep >= 0) {
				if (stepIt[currentStep].hasNext()) {
 					StructureAssign currentAssign = (StructureAssign) stepIt[currentStep].next();
					stepAssign[currentStep] = currentAssign;
					if (currentStep == numberOfSteps) {
						totalAnswer.add(currentAssign.structure);
					}
					else {
						FocusStep step = (FocusStep) conjunction.get(currentStep);
						currentStep++;
						stepIt[currentStep] = step.focus(new StructureAssign(currentAssign)).iterator();
					}
				}
				else {
					currentStep--;
...
class StructureAssign {
	...
	/** Constructs a StructureAssign object by creating a deep copy of the specified
	 * object.
	 * @param other The StructureAssign to copy into this object.
	 */
	...
	/** Constructs a StructureAssign object from the specified specified structure and
	 * a list of assignments.
	 * @param structure A structure.
	 * @param assigns A list of Assign objects.
	 */
	...
	/** Constructs a StructureAssign object from the specified structure and a single
	 * assignment.
	 * @param structure A structure.
	 * @param newAssign An assignment.
	 */
	...
	/** Constructs a StructureAssign object from the specified structure and an iterator
	 * to a list of assignments. This constructor does a deep-copy of the specified
	 * list.
	 * @param structure A structure.
	 * @param newAssign An assignment.
	 */
	...
	/** Constructs a StructureAssign object from the specified structure ans assignments.
	 * Both the list of assignments and the additional assignment are duplicated.
	 * @param structure A structure.
	 * @param oldAssigns A list of assignments.
	 * @param newAssign An optional additional assignment. This argument may be null.
	 */
	...
	/** Projects the assignments associated with the structure to the set of relevant
	 * variables.
	 * @param variables The variables used to project each assignment.
	 */
	...
	public String toString() {
		return StructureToTVS.defaultInstance.convert(structure, assignsToString());
	}
...
class FocusStep {
	...
	/** Indicates whether a warning message has been emitted for a cases which can cause
	 * an infinite numebr of structures to arise.
	 */	
	protected static boolean focusWarningEmitted = false;
	
	/** Constructs and initializes a focus step.
	 * @param literal The literal that the step concentrates on focusing.
	 */
	...
	public String toString() {
		return "literal=" + literal + ", future=" + 
			   futureVariables + ", now and future=" + nowAndFutureVariables;
	}
	...
	public Collection<TVS> focus(TVS structure, Assign assign) {
		...
		case 2:
			if (assign.isEmpty()) // a special case for unbound binary predicate formulae
				...
				return focusBinaryInvFunction(structure, predicateFormula, assign);
			else
				return focusBinaryNonFunction(structure, predicateFormula, assign);
	...
	public Collection<StructureAssign> generateAssign(Collection structures, 
		...
		if (newAndNeeded.isEmpty()) {
			for (Iterator structureIt = structures.iterator();
				 structureIt.hasNext(); ) {
	            ...
	            Iterator values = structure.evalFormulaForValue(literal.atomic, relevant,
                        literal.negated ? 
                        Kleene.falseKleene : 
                        Kleene.trueKleene);
	            if (!values.hasNext()) {
				...
				StructureAssign structureAssign = new StructureAssign(structure, previousAssigns);
		...
		for (Iterator structureIt = structures.iterator();
			...
			Iterator afterStep = structure.evalFormulaForValue(literal.atomic, relevant,
															   literal.negated ? 
															   Kleene.falseKleene : 
															   Kleene.trueKleene);
			...
			while (afterStep.hasNext()) {
				...
				for (Iterator<Assign> prevIt = previousAssigns.iterator(); 
					 prevIt.hasNext(); ) {
	...
	public Collection<StructureAssign> focus(StructureAssign current) {
		...
		if (current.assigns.isEmpty()) {
			answer.add(new StructureAssign(current.structure, new ArrayList<Assign>()));
			return answer;
		}

		Map<Assign, Collection<TVS>> relevantAssignToStructures = HashMapFactory.make();
		Map<Assign, List<Assign>> relevantAssignToFullAssign = HashMapFactory.make();

		for (Iterator<Assign> assignIt = current.assigns.iterator(); assignIt.hasNext(); ) {
			...
			if (fullAssigns == null) { // Unknown relevant assignment!		
				...
				// First focus on active for new variables that might satisfy the
				// formula.
				Collection<TVS> activeFocused = focusActive(current.structure, relevant);		
		...
		for (Map.Entry<Assign, List<Assign>> entry : relevantAssignToFullAssign.entrySet()) {
	...
	/** Creates all the structures that have the specified nodes as either active or
	 * non-active (all the binary combinations of active/non-active for these nodes).
	 * There are 2^(|toFocus|) such structures.
	 * @param toFocus A collection of maybe-active nodes.
	 * postcondition: foreach s in return 
	 *           	    foreach n in toFocus
	 *                	  s.eval(Vocabulary.active, n) != Kleene.unknownKleene
	 * @author Tal Lev-Ami
	 */
	private Collection<TVS> focusActive(TVS structure, Collection<Node> toFocus) {
		...
		answer.add(structure); // start with a work set containing only this structure
		for (Iterator<Node> nodeIt = toFocus.iterator(); nodeIt.hasNext(); ) {
			...
			for (TVS currentStructure : answer) {
				if (inScope(currentStructure, problemNode) && 
				    currentStructure.eval(Vocabulary.active, problemNode) == Kleene.unknownKleene) {
	 ...
	 * @author Tal Lev-Ami
	 */
	private Collection<TVS> focusActiveUnique(TVS structure, Collection<Node> toFocus) {
		List<TVS> answer = new ArrayList<TVS>();
		...
		for (Iterator<Node> nodeIt = toFocus.iterator(); nodeIt.hasNext(); ) {
			Node problemNode = nodeIt.next();
            if (!inScope(structure, problemNode)) continue;
	...
	private Collection<TVS> focusActive(TVS structure, Assign assign) {
		...
		while (problemIt.hasNext()) {
			...
			for (Iterator<Var> varIt = newVariables.iterator(); varIt.hasNext(); ) {
                ...
                if (!inScope(structure, problemNode)) continue;
				if (structure.eval(Vocabulary.active, problemNode).equals(Kleene.unknownKleene))
	...
	private Collection<TVS> focusUnaryUnique(TVS structure, 
		...
		Iterator<AssignKleene> problemIt = structure.evalFormulaForValue(formula, assign, Kleene.unknownKleene);
		...
		Iterator<AssignKleene> hasTrue = structure.evalFormulaForValue(formula, assign, Kleene.trueKleene);        
		if (hasTrue.hasNext()) {
	        TVS without = structure.copy();
		    while (problemIt.hasNext()) {
                Node problemNode = problemIt.next().get(formula.getVariable(0));
                without.update(formula.predicate(), problemNode, Kleene.falseKleene);
		    }
		    answer.add(without);
		    return answer;
		}
		...
		while (problemIt.hasNext()) {
			...
			if (without.eval(Vocabulary.sm, problemNode).equals(Kleene.unknownKleene)) {
				// Add new structure where problemNode is bifurcated into two nodes.
				TVS structureWithDuplicatedNode = without.copy();		

				Node newNodeTrue = structureWithDuplicatedNode.duplicateNode(problemNode);
				structureWithDuplicatedNode.update(formula.predicate(), newNodeTrue,
										 Kleene.trueKleene);
				structureWithDuplicatedNode.update(formula.predicate(), problemNode,
										 Kleene.falseKleene);
				
				// This is a unique predicate so the true node is not a summary node.
				structureWithDuplicatedNode.update(Vocabulary.sm, newNodeTrue, Kleene.falseKleene);
				ModifiedPredicates.modify(structureWithDuplicatedNode, Vocabulary.sm);
				ModifiedPredicates.modify(structureWithDuplicatedNode, formula.predicate());
	 ...
	 */
	private Collection<TVS> focusUnaryNonUnique(TVS structure, PredicateFormula formula, Assign assign) {
		List<TVS> workSet = new ArrayList<TVS>();
		List<TVS> answer = new ArrayList<TVS>();
		...
		while (!workSet.isEmpty()) {
			...
			Iterator problemIt = currentStructure.evalFormulaForValue(ensureInScope(formula), assign, Kleene.unknownKleene);
			if (!problemIt.hasNext()) {
			...
			// Case 1: p evaluates to false on all of the concrete nodes represented by problemNode.
			// Add a new structure where p evaluates to false on problemNode.
			TVS withoutEdge = currentStructure.copy();
			withoutEdge.update(formula.predicate(), problemNode, Kleene.falseKleene);
			ModifiedPredicates.modify(withoutEdge, formula.predicate());
			workSet.add(withoutEdge);

			// Case 2: p evaluates to true on all of the concrete nodes represented by problemNode.
			...
			// Case 3: p evaluates to true on some of the concrete nodes represented by problemNode
			// and to false on the others.
			// If the node is a summary node, bifurcate it into two summary nodes
			...
			if (currentStructure.eval(Vocabulary.sm, problemNode) == Kleene.unknownKleene) {
				...
				// Duplicate the problem node - p will evaluate to true on the cloned node.				
				Node newNodeTrue = structureWithDuplicatedNode.duplicateNode(problemNode);				
				structureWithDuplicatedNode.update(formula.predicate(), newNodeTrue, Kleene.trueKleene);
				structureWithDuplicatedNode.update(formula.predicate(), problemNode, Kleene.falseKleene);
				ModifiedPredicates.modify(structureWithDuplicatedNode, formula.predicate());
	...
	private void focusBinary(Collection<TVS> answer, TVS structure,
							 Predicate predicate,
							 Node leftProblem, Node rightProblem) {
		boolean leftSummary = structure.eval(Vocabulary.sm, leftProblem).equals(Kleene.unknownKleene);
		boolean rightSummary = structure.eval(Vocabulary.sm, rightProblem).equals(Kleene.unknownKleene);

		// Dectect infinite Focus conditions.
		if (leftSummary && rightSummary) {
			// This is a real problem - focus will cause an infinite number of structure!
			// Don't give-up on the whole analysis, just return a less precise anwer.
			...
			String message = "\nError! Focusing on predicate " + predicate +
							 ...
							 "\non the edge from " + leftProblem.name() +
							 " to " + rightProblem.name();
		...
		if (rightSummary) {
			// Add new structure where rightProblem is bifurcated into two nodes.
			TVS structureWithDuplicatedNode = structure.copy();
			Node rightProblemTrue = structureWithDuplicatedNode.duplicateNode(rightProblem);
			structureWithDuplicatedNode.update(predicate, leftProblem, rightProblemTrue, 
									  Kleene.trueKleene);
			structureWithDuplicatedNode.update(predicate, leftProblem, rightProblem,
									  Kleene.falseKleene);
			ModifiedPredicates.modify(structureWithDuplicatedNode, predicate);
			// If the predicate is function the right node must be non summary.
			// (since the left node is a summary node).
			if (predicate.function()) {
				structureWithDuplicatedNode.update(Vocabulary.sm, rightProblemTrue, Kleene.falseKleene);
				ModifiedPredicates.modify(structureWithDuplicatedNode, Vocabulary.sm);
			}
			answer.add(structureWithDuplicatedNode);
		}
		if (leftSummary) {
			...
			TVS structureWithDuplicatedNode = structure.copy();
			Node leftProblemTrue = structureWithDuplicatedNode.duplicateNode(leftProblem);
			structureWithDuplicatedNode.update(predicate, leftProblemTrue, rightProblem, 
									  Kleene.trueKleene);
			structureWithDuplicatedNode.update(predicate, leftProblem, rightProblem,
									  Kleene.falseKleene);
			ModifiedPredicates.modify(structureWithDuplicatedNode, predicate);

			// If the predicate is invfunction the leftt node must be non summary.
			// (since the right node is a summary node).
			if (predicate.invfunction()) {
				structureWithDuplicatedNode.update(Vocabulary.sm, leftProblemTrue, Kleene.falseKleene);
				ModifiedPredicates.modify(Vocabulary.sm);
			}
			answer.add(structureWithDuplicatedNode);
		}
	...
	private void focusKary(Collection<TVS> answer, TVS structure,
		...
		for (int index = 0; index < problems.size(); ++index) {
			...
			structureWithDuplicatedTuple.update(predicate,
												problems.substitute(node, problemTrue), 
												Kleene.trueKleene);
			structureWithDuplicatedTuple.update(predicate, problems, Kleene.falseKleene);
			ModifiedPredicates.modify(structureWithDuplicatedTuple, predicate);

			// If the predicate is function the true node must be non summary.
			// (since the left node can't be a summary node).
			//if (predicate.function())
			//	structureWithDuplicatedTuple.update(Vocabulary.sm, node, Kleene.falseKleene);
			answer.add(structureWithDuplicatedTuple);
		}
	}

	/** Returns a set of structures formed by focusing the specified structure on the
	 * binary predicate with all indefinite edges. If the structure contains a summary
	 * node with an unknown edge (of the specified predicate) then at most a single
	 * node bifurcation may occur to insure termination.
	 */
	private Collection<TVS> focusBinaryUnbound(TVS structure,
		...
		Iterator problemIt = structure.evalFormulaForValue(ensureInScope(bformula), Assign.EMPTY, Kleene.unknownKleene);
		if (!problemIt.hasNext())
			return Collections.singleton(structure);
		...
		while (problemIt.hasNext()) {
			Assign problemAssign = (Assign) problemIt.next();
			if (function) {
				answer.addAll(focusBinaryFunction(structure, bformula, problemAssign));
			}
			else if (invfunction) {
				answer.addAll(focusBinaryInvFunction(structure, bformula, problemAssign));
			}
			else {
				answer.addAll(focusBinaryNonFunction(structure, bformula, problemAssign));
			}
		}
		return answer;
	}
	...
	/** Returns a set of structures formed by focusing the specified structure on the
	 * binary function predicate with a partial assignment.
	 * @author Tal Lev-Ami
	 * @param assign A partial assignment.
	 */
	private Collection<TVS> focusBinaryFunction(TVS structure,
										   PredicateFormula formula, 
										   Assign assign) {
										   	
 		// Roman, May 9, 2001 : Added special treatment for definite edges pointing to 
  		// maybe-active nodes to solve a bug from TVLA version 0.9.
  		//
  		// Roman, January 9, 2004: Fixed a bug from TVLA version 0.9, which resulted
  		// in removal of too many edges in cases where the indefinite edge is a self-loop
  		// (only the self-loop edge was refined and the rest were removed).
  												   	
        Formula scopedFormula = ensureInScope(formula);
		List<TVS> answer = new ArrayList<TVS>();
		Iterator problemIt = structure.evalFormulaForValue(scopedFormula, assign, Kleene.unknownKleene);
		if (!problemIt.hasNext()) {
			answer.add(structure);
			return answer;
		}
		...
		if (leftProblem == null) {
			throw new RuntimeException("Left node is supposed to be bound when " +
		...
		}
		{
            ...
            if (rightProblem != null) {
                ...
                // For without - simply remove the edge and return - we know nothing of the other edges
                TVS without = structure.copy();
                without.update(predicate, leftProblem, rightProblem, Kleene.falseKleene);
                ...
                Iterator<Entry<NodeTuple, Kleene>> iterator = structure.predicateSatisfyingNodeTuples(predicate, partial, Kleene.unknownKleene);
                PredicateUpdater updater = PredicateUpdater.updater(predicate, other);
                while (iterator.hasNext()) {
		...
		for (Node node : structure.nodes()) {
			...
			if (edgeVal == Kleene.unknownKleene) {
                ...
                if (inScope(structure, node)) {
    				withoutSafe.update(predicate, leftProblem, node, Kleene.falseKleene);
    				ModifiedPredicates.modify(withoutSafe, formula.predicate());
                }
			}
				...
				assert without.eval(Vocabulary.active, node) == Kleene.unknownKleene :
				"Attempt to focus on definite edge where individual on other end " +
		...
		if (!hasDefiniteEdge) {
			...
			for (Node rightProblem : structure.nodes()) {
				if (structure.eval(predicate, leftProblem, rightProblem) == Kleene.unknownKleene &&
					...
					focusBinary(answer,	without, predicate,	leftProblem, rightProblem);
				}
			}
		}
		return answer;
	...
	/** Returns a set of structures formed by focusing the specified structure on the
	 * binary inverse-function predicate with a partial assignment.
	 * @param assign A partial assignment.
	 * @author Roman Manevich
	...
	private Collection<TVS> focusBinaryInvFunction(TVS structure,
		...
		Node rightProblem = assign.get(formula.getVariable(1));
		if (rightProblem == null) {
			throw new RuntimeException("Right node is supposed to be bound when " +
				"focusing on predicate " + formula + " on structure " + structure);
		}
		...
		for (Iterator nodeIt = structure.nodes().iterator(); nodeIt.hasNext(); ) {
			...
			Kleene edgeVal = without.eval(formula.predicate(), node, rightProblem);
			if (edgeVal == Kleene.unknownKleene) {
				without.update(formula.predicate(), node, rightProblem, Kleene.falseKleene);
				ModifiedPredicates.modify(without, formula.predicate());
				//ModifiedPredicates.modify(formula.predicate());
			}
			else if (edgeVal == Kleene.trueKleene) {
		...
		if (!hasDefiniteEdge) {
			while (problemIt.hasNext()) {
				...
				focusBinary(answer, without, formula.predicate(), leftProblem, rightProblem);
			}
		}
		return answer;
	}
	...
	private Collection<TVS> focusBinaryNonFunction(TVS structure, 
		...
		while (!workSet.isEmpty()) {
			...
			Iterator problemIt = currentStructure.evalFormulaForValue(scopedFormula, assign, Kleene.unknownKleene);
			if (!problemIt.hasNext()) {
				answer.add(currentStructure);
				continue;
			}
			...
			if (!(formula.predicate().reflexive() && leftProblem.equals(rightProblem))) {
				// Add new structure where n(leftProblem, rightProblem) = 0
				TVS withoutEdge = currentStructure.copy();
				withoutEdge.update(formula.predicate(), leftProblem, rightProblem, Kleene.falseKleene);
				ModifiedPredicates.modify(withoutEdge, formula.predicate());

				workSet.add(withoutEdge);
			}
			focusBinary(workSet, currentStructure, formula.predicate(), leftProblem, rightProblem);
		}
		ModifiedPredicates.modify(formula.predicate());
		return answer;
	}
    ...
    private Formula ensureInScope(Formula formula) {
        for (Var var : formula.freeVars()) {
            formula = new AndFormula(new NotFormula(new PredicateFormula(Vocabulary.outside, var)), formula);
        }
        return formula;
    }
    ...
    protected void safeClearPredicate(TVS structure, Predicate predicate) {
        Iterator<Entry<NodeTuple, Kleene>> outsideNodes = structure.predicateSatisfyingNodeTuples(Vocabulary.outside, new Node[0], Kleene.trueKleene);
        if (outsideNodes.hasNext()) {
            Var v = new Var("v");
            Iterator<AssignKleene> insideNodes = structure.evalFormulaForValue(new NotFormula(new PredicateFormula(Vocabulary.outside, v)), Assign.EMPTY, Kleene.trueKleene);
            while (insideNodes.hasNext()) {
                structure.update(predicate, insideNodes.next().get(v), Kleene.falseKleene);
            }
        } else {
...
class FocusFormula {
	 ...
	 * optimal order of literals.
	 * The list is used for emitting an informative error message.
	 */	
	private static List<FocusStep> unboundBinaryPredicates = null;

	/** Constructs a new formula by optimizing the specified formula.
	 * @param formula The formula to optimize.
	 */
	public FocusFormula(Formula formula) throws FocusNonTerminationException {
		...
		// For each dijunction put its literals in a list and insert it to the 
		// focusFormulae list.
		...
		for (int i = 0; i < orTerms.size(); i++) {
			...
			List<FocusStep> unbound = calculateOptimalOrder(andTerms, focusFormulae[i], formula);
			if (!unbound.isEmpty()) {
		...
		if (encounteredConjunctionWithUnbounded) {
			String message =
				"Warning! Simplifying the Focus formula "
					+ formula
					+ "\n"
	 ...
	 */
	private List<FocusStep> calculateOptimalOrder(List<Formula> terms, List<FocusStep> bound, Formula origFormula) {
			...
			if (literal.atomic instanceof PredicateFormula) {
				...
				if (qformula.left().equals(qformula.right())) {
					...
					if (!AnalysisStatus.terse)
						throw new FocusNonTerminationException("Trying to focus on non trivial equality will" +
							" lead to" +
		...
		while (change) {
			change = false;
			for (Iterator<FocusStep> moreUnbound = unbound.iterator(); moreUnbound.hasNext(); ) {
				FocusStep step = moreUnbound.next(); 
				Literal literal = step.literal;
				if (literal.atomic instanceof PredicateFormula &&
					((PredicateFormula) literal.atomic).predicate().arity() == 2) {
					...
					if (boundVariables.contains(bformula.getVariable(0))) {
						...
						change = true;
						moreUnbound.remove();
						if (bformula.predicate().invfunction() && !literal.negated)
							boundVariables.add(bformula.getVariable(0));
					}
				} 
				else { // internal error
					throw new RuntimeException("Unknown atomic formula " + literal.atomic);
				}
			}
		}
		...
		for (Iterator<FocusStep> moreUnbound = unbound.iterator(); moreUnbound.hasNext(); ) {
			...
			Literal literal = step.literal;
			PredicateFormula bformula = (PredicateFormula) literal.atomic;
			if ((bformula.predicate().function() || bformula.predicate().invfunction()) &&
				!literal.negated) {
				binary.add(step);
				moreUnbound.remove();
			}
	...
	public String toString() {
		...
		for (int i=0; i < focusFormulae.length; ++i) {
			...
			out.append("(");
			for (Iterator conjuctionIterator = conjunction.iterator();
				 conjuctionIterator.hasNext(); ) {
