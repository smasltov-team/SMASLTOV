...
//                   System.err.print(name + " " + c + "    ");
                   ...
                   try {
                       for (HighLevelTVS structure : structures) {
                    	   ...
                    	   Map<HighLevelTVS, Collection<String>> messages = HashMapFactory.make(0);
                           //Collection<HighLevelTVS> results = apply(currentAction, structure, currentLocation.label(), messages);
                           Collection<HighLevelTVS> results = apply(currentAction, structure, currentLocation.label(), null); // Changed by Jason Breck in 2014-04
                           if (Engine.coerceAfterUpdateFailed || !messages.isEmpty()) {
                               ...
                               apply(currentAction, structure,
                                       currentLocation.label(), null); // changed by Jason Breck in 2014-04
                               AnalysisStatus.debug = debug;
                           }
                           //status.numberOfMessages += getProcessedLocation().addMessages(messages);
                           status.numberOfMessages += getProcessedLocation().addMessages(null); // Changed by Jason Breck in 2014-04
    ...
    private void replay(CompositionStrategy strategy, CartesianElement beforeOld, CartesianElement beforeDelta, CartesianElement beforeNew) {
        ...
        for (Pair<DecompositionName, Iterable<HighLevelTVS>> component : strategy) {
            ...
            for (HighLevelTVS structure : structures) {
                Map<HighLevelTVS, Collection<String>> messages = HashMapFactory.make(0);
                //apply(currentAction, structure, currentLocation.label(), messages); // Changed by Jason Breck in 2014-04
                apply(currentAction, structure, currentLocation.label(), null);
            }
        }
        AnalysisStatus.debug = debug;
        strategy.done();
    ...
    protected boolean reportMessages(Action action, HighLevelTVS structure, Assign assign,
            Map<HighLevelTVS, Collection<String>> messages) {
        Collection<String> newMessages = reportMessages(action, structure, assign, action.getMessages());
    ...
    */
    ...
    private Collection<String> reportMessages(Action action, HighLevelTVS structure, Assign assign, Collection<ReportMessage> messages) {
        Collection<String> newMessages = new ArrayList<String>();
...
//                action.checkedMessage(message);
    ...
    protected void reportPostMessages(Action action, HighLevelTVS structure,
        ...
        Collection<String> postMsgs = reportMessages(action, blurred, Assign.EMPTY, action.getPostMessages());
        if (!postMsgs.isEmpty()) {
