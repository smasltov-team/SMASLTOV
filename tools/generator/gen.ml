...
and gen_ funcstr id0 flist =
  let idlist =  
	...
	[] -> [id0]
      ...
      | _ -> failwith "List too long in idlist"
  in 
  ...
  let childlist = 
	...
	[] -> []
      ...
      | _ -> failwith "List too long in childlist"
  in
...
let update_union_1 id0 id1 id2 lbl_in proof = 
  let lbl_out = nxtlbl() in
  (lbl_out, proof ^ (lstr lbl_out)^" = UpdateUnion" ^ args2str_sq [hstr id0; hstr id1; hstr id2] ^args2str[(lstr lbl_in)] ^ "\n")

let update_union_2 id0 id1 id2 lbl_in proof = 
  let lbl_out = nxtlbl() in
  (lbl_out, proof ^ (lstr lbl_out)^" = UpdateUnionAgain" ^ args2str_sq [hstr id0; hstr id1; hstr id2] ^args2str[(lstr lbl_in)] ^ "\n")
;;
...
let havoc_partition id0 id1 id2 lbl_in proof = 
  ...
  let (c4, proof) = update_union_1 id0 id1 id2 c3 proof in
  let (c5, proof) = update_union_2 id0 id1 id2 c4 proof in
  project_region id0 id1 id2 c5 proof
;;
...
let update_region id0 id1 id2 lbl_in proof = 
  ...
  let u2, proof = update_region_2 id0 id1 id2 u1 proof in
  (u2, proof)
;;
...
let new_proof_rules f = 
  ...
  (* Setup *)
  reset_lblcount(); 
  let id0 = 0 in (* This is the id of the heap region. By default, we are calculating for d0. *)
  let proof = "" in
  ...
  (* Refine the underapproximation using the overapproximation *)
  (* This is required only because we use different over and under approximations 
     for the leaves. They have the same meaning, but the underapproximations
     are higher in the lattice. *)
  let pu, proof = 
    if rpo & rpu then meet po pu proof
    else (pu, proof) in
  let nu, proof = 
    if rno & rnu then meet no nu proof
    else (nu, proof) in
...
let batch_element_proof_rules f fname clear_cache = 
  (* print the proof rules for a single formula that's an element of a batch *)
  let splt = false in
  let rpo = true in
  let rpu = false in
  let rno = false in
  let rnu = false in
  (* Setup *)
  (* reset_lblcount(); *) (* don't reset, so that each batch element gets separate location labels *)
  let id0 = 0 in (* This is the id of the heap region. By default, we are calculating for d0. *)
  let proof = "" in
  (* May want to experiment with not flushing the cache, in the future: *)
  (if clear_cache then 
     (Hashtbl.clear rpo_cache;
      Hashtbl.clear rpu_cache;
      Hashtbl.clear rno_cache;
  ...
  let proof = proof ^ (if rpo then "// Outcome: " ^ fname ^ " PosOver@ "^ (lstr po) ^ "\n" else "") in
  let proof = proof ^ (if rpu then "// Outcome: " ^ fname ^ " PosUnder@ "^(lstr pu) ^ "\n" else "") in
  let proof = proof ^ (if rno then "// Outcome: " ^ fname ^ " NegOver@ "^ (lstr no) ^ "\n" else "") in
  let proof = proof ^ (if rnu then "// Outcome: " ^ fname ^ " NegUnder@ "^(lstr nu) ^ "\n" else "") in
